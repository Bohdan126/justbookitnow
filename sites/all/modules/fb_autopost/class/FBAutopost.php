...
class FBAutopost extends Facebook {
  ...
  function __construct($conf = array()) {
    if (!isset($conf['app_id'])) {
      $conf['app_id'] = variable_get('fb_autopost_app_id', '');
    }
    if (!isset($conf['app_secret'])) {
      $conf['app_secret'] = variable_get('fb_autopost_app_secret', '');
    }
    if (!isset($conf['default_graph_version'])) {
      $conf['default_graph_version'] = 'v2.5'; // TODO make graph version a variable
    }
    ...
    try {
      ...
      if ($this->getRetry()) {
        ...
        if ($page_id == 'me' && !$session->isStored()) {
          ...
          $login_url = $this->getRedirectLoginHelper()->getLoginUrl(
            url('fbautopost/authorization/retry', array('absolute' => TRUE)),
            fb_permissions_get_facebook_permissions()
          );
  ...
  protected function publishOn($publication) {
    ...
    $result = $this->post(
      ...
      $publication['params'],
      // Add access token.
      $publication['params']['access_token']
    );
  ...
  protected function publishParameterPrepare(&$publication) {
    ...
    if ($destination != 'me') {
	  ...
	  $publication['params'] += array('access_token' => variable_get('fb_autopost_token', ''));
  ...
  public function getPagesData($account_id, $account_access_token) {
    ...
    try {
      return $this->get('/' . $account_id . '/accounts', $account_access_token)->getDecodedBody();
   ...
   *   The id of the publication in Facebook
   * * @param $access_token
   *   An access token that is allowed to delete the publication
   * @throws FacebookApiException
   */
  public function remoteDelete($publication_id, $access_token) {
    // TODO: Add a remoteDelete callback to the entity delete (optional)
    $this->delete('/' . $publication_id, array(), $access_token);
