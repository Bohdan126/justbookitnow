...
function bizreview_preprocess_field(&$variables, $hook) {
  global $user;
  $node = menu_get_object();
  if ($variables['element']['#field_name'] === 'field_booking_details') {
    $field = $variables['element'][0]['#markup'];
    $field .= '<a class="btn btn-primary" target="_blank" href="'. $node->field_ticket_link[LANGUAGE_NONE][0]['url'] . '">JUST BOOK IT NOW</a>';
    $variables['items'][0]['#markup'] = $field;
  }
}
